#!/usr/bin/env bash
# ============================================================
# KyuzenOS — smoke test LLVM libc 22.1.8 Phase 2 (stdio) di QEMU (otomatis)
#
# Alur (tanpa interaksi manusia):
#   1. disk image BARU (build/libc/phase2-disk.img) → boot pertama meminta
...
#   3. app dijalankan dari console shell: `start libc_phase2`
...
#
# Dipakai oleh `make libc-phase2-qemu`; bisa juga dijalankan langsung:
#   QEMU=/e/Tools/msys2/qemu/qemu-system-x86_64.exe bash tools/libc-phase2/run-qemu.sh
#
...
OUT="build/libc"
DISK="$OUT/phase2-disk.img"
SERIAL="$OUT/phase2-serial.log"
MONLOG="$OUT/phase2-qemu-monitor.log"
ISO="build/boot_image.iso"
APP="$OUT/x86_64-pc-none-elf/bin/libc_phase2.elf"

ROOT_PASS="${KYUZEN_TEST_PASS:-kz}"
APP_NAME="${KYUZEN_TEST_APP:-libc_phase2}"
...
for f in "$ISO" "$APP"; do
    [ -f "$f" ] || { echo "[qemu] FATAL: $f tidak ada (make libc-phase2-qemu yang menyiapkannya)" >&2; exit 1; }
done
...
feed_keys() {
    ...
    for _ in $(seq 1 120); do
        grep -qF "[phase2] PASS" "$SERIAL" 2>/dev/null && return 0
        grep -qF "[phase2] FAIL" "$SERIAL" 2>/dev/null && return 0
        sleep 2
    done
    return 1
}
...
while kill -0 "$QPID" 2>/dev/null; do
    if grep -qE "\[phase2\] (PASS|FAIL)" "$SERIAL" 2>/dev/null; then
...
if grep -qF "[phase2] PASS" "$SERIAL"; then
    echo "[qemu] PASS terlihat di serial"
    exit 0
fi

echo "[qemu] FAIL: '[phase2] PASS' tidak muncul di serial" >&2
