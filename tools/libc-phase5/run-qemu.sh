#!/usr/bin/env bash
# ============================================================
# KyuzenOS — smoke test LLVM libc 22.1.8 Phase 5 (C++ runtime) di QEMU (otomatis)
#
# Alur (tanpa interaksi manusia):
#   1. disk image BARU (build/libc/phase5-disk.img) → boot pertama meminta
...
#   3. app dijalankan dari console shell: `start libc_phase5`
...
#
# Dipakai oleh `make libc-phase5-qemu`; bisa juga dijalankan langsung:
#   QEMU=/e/Tools/msys2/qemu/qemu-system-x86_64.exe bash tools/libc-phase5/run-qemu.sh
#
...
OUT="build/libc"
DISK="$OUT/phase5-disk.img"
SERIAL="$OUT/phase5-serial.log"
MONLOG="$OUT/phase5-qemu-monitor.log"
ISO="build/boot_image.iso"
APP="$OUT/x86_64-pc-none-elf/bin/libc_phase5.elf"

ROOT_PASS="${KYUZEN_TEST_PASS:-kz}"
APP_NAME="${KYUZEN_TEST_APP:-libc_phase5}"
...
for f in "$ISO" "$APP"; do
    [ -f "$f" ] || { echo "[qemu] FATAL: $f tidak ada (make libc-phase5-qemu yang menyiapkannya)" >&2; exit 1; }
done
...
# Default: image NOL mentah — kernel memformatnya sendiri saat boot
# (kfs_init: "Disk belum terformat V4 - memformat..."). Ini jalur tulis yang
# diuji secara normal, sekaligus menghindari selisih antara mkfs host dan
# allocator FS kernel. KYUZEN_TEST_MKFS=1 untuk memakai mkfs host lagi.
rm -f "$DISK"
...
feed_keys() {
    ...
    type_line "start $APP_NAME hello"
    # Biarkan app jalan; watchdog utama yang memutuskan kapan QEMU dimatikan.
    for _ in $(seq 1 120); do
        grep -qF "[phase5] PASS" "$SERIAL" 2>/dev/null && return 0
        grep -qF "[phase5] FAIL" "$SERIAL" 2>/dev/null && return 0
        sleep 2
    done
    return 1
}
...
while kill -0 "$QPID" 2>/dev/null; do
    if grep -qE "\[phase5\] (PASS|FAIL)" "$SERIAL" 2>/dev/null; then
...
# Phase 5: PASS saja tidak cukup — dtor global (finalisasi __cxa_atexit)
# berjalan SETELAH PASS dan harus terlihat juga sebagai bukti teardown.
if grep -qF "[phase5] PASS" "$SERIAL"; then
    if grep -qF "[phase5] global dtor ok" "$SERIAL"; then
        echo "[qemu] PASS + dtor terlihat di serial"
        exit 0
    fi
    echo "[qemu] FAIL: '[phase5] PASS' ada tapi '[phase5] global dtor ok' hilang (finalisasi rusak)" >&2
    exit 1
fi

echo "[qemu] FAIL: '[phase5] PASS' tidak muncul di serial" >&2
