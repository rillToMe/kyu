#!/usr/bin/env bash
# ============================================================
# KyuzenOS — smoke test LLVM libc 22.1.8 Phase 6 (libc++ subset) di QEMU (otomatis)
#
...
#
# Dipakai oleh `make libc-phase6-qemu`; bisa juga dijalankan langsung:
#   QEMU=/e/Tools/msys2/qemu/qemu-system-x86_64.exe bash tools/libc-phase6/run-qemu.sh
#
...
OUT="build/libc"
DISK="$OUT/phase6-disk.img"
SERIAL="$OUT/phase6-serial.log"
MONLOG="$OUT/phase6-qemu-monitor.log"
ISO="build/boot_image.iso"
APP="$OUT/x86_64-pc-none-elf/bin/libc_phase6.elf"

ROOT_PASS="${KYUZEN_TEST_PASS:-kz}"
APP_NAME="${KYUZEN_TEST_APP:-libc_phase6}"
...
for f in "$ISO" "$APP"; do
    [ -f "$f" ] || { echo "[qemu] FATAL: $f tidak ada (make libc-phase6-qemu yang menyiapkannya)" >&2; exit 1; }
done
...
feed_keys() {
    ...
    for _ in $(seq 1 120); do
        grep -qF "[phase6] PASS" "$SERIAL" 2>/dev/null && return 0
        grep -qF "[phase6] FAIL" "$SERIAL" 2>/dev/null && return 0
        sleep 2
    done
    return 1
}
...
while kill -0 "$QPID" 2>/dev/null; do
    if grep -qE "\[phase6\] (PASS|FAIL)" "$SERIAL" 2>/dev/null; then
...
if grep -qF "[phase6] PASS" "$SERIAL"; then
    echo "[qemu] PASS terlihat di serial"
    exit 0
fi

echo "[qemu] FAIL: '[phase6] PASS' tidak muncul di serial" >&2
