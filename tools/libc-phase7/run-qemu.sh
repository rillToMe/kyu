#!/usr/bin/env bash
# ============================================================
# KyuzenOS — smoke test C++ Application SDK Phase 7 di QEMU (otomatis)
#
# Alur (tanpa interaksi manusia):
#   1. disk image BARU (build/libc/phase7-disk.img) → boot pertama meminta
...
#   3. app dijalankan dari console shell: `start libc_phase7 hello`
...
#
# Dipakai oleh `make libc-phase7-qemu`; bisa juga dijalankan langsung:
#   QEMU=/e/Tools/msys2/qemu/qemu-system-x86_64.exe bash tools/libc-phase7/run-qemu.sh
#
...
#                    KYUZEN_TEST_APP_PATH, KYUZEN_TEST_DISK, KYUZEN_TEST_SERIAL,
#                    KYUZEN_TEST_MONLOG, KYUZEN_TEST_MARKER,
#                    KYUZEN_TEST_SUCCESS, KYUZEN_TEST_FAILURE
# ============================================================
set -uo pipefail
...
OUT="build/libc"
DISK="${KYUZEN_TEST_DISK:-$OUT/phase7-disk.img}"
SERIAL="${KYUZEN_TEST_SERIAL:-$OUT/phase7-serial.log}"
MONLOG="${KYUZEN_TEST_MONLOG:-$OUT/phase7-qemu-monitor.log}"
ISO="build/boot_image.iso"
APP="${KYUZEN_TEST_APP_PATH:-$OUT/x86_64-pc-none-elf/bin/libc_phase7.elf}"
MARKER="${KYUZEN_TEST_MARKER:-[phase7]}"
SUCCESS="${KYUZEN_TEST_SUCCESS:-$MARKER PASS}"
FAILURE="${KYUZEN_TEST_FAILURE:-$MARKER FAIL}"

ROOT_PASS="${KYUZEN_TEST_PASS:-kz}"
APP_NAME="${KYUZEN_TEST_APP:-libc_phase7}"
...
for f in "$ISO" "$APP"; do
    [ -f "$f" ] || { echo "[qemu] FATAL: $f tidak ada (make libc-phase7-qemu yang menyiapkannya)" >&2; exit 1; }
done
...
feed_keys() {
    ...
    for _ in $(seq 1 120); do
        grep -qF "$SUCCESS" "$SERIAL" 2>/dev/null && return 0
        grep -qF "$FAILURE" "$SERIAL" 2>/dev/null && return 0
        sleep 2
    done
    return 1
}
...
while kill -0 "$QPID" 2>/dev/null; do
    if grep -qF -- "$SUCCESS" "$SERIAL" 2>/dev/null || grep -qF -- "$FAILURE" "$SERIAL" 2>/dev/null; then
        sleep 4
        break
    fi
...
if grep -qF -- "$SUCCESS" "$SERIAL"; then
    echo "[qemu] sukses terlihat di serial: $SUCCESS"
    exit 0
fi
