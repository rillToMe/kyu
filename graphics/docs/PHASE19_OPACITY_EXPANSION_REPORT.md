...
apps/libgui.c — after the initial full-canvas paint + upload, each window
...
- `Painter::image` -> PNG pixels from apps/png.c, which forces `0xFF000000`
...
- apps/userlib.c — implemented the syscall 67 wrapper.
- apps/libgui.c — call `sys_kwm_set_window_opaque()` after the initial opaque
